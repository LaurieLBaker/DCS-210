---
title: "Lab 00 - La Quinta is Spanish for next to Denny's, Pt. 1"
...
In fact, la quinta translated to Spanish means "the fifth".
...
-   Visualising spatial data in leaflet
-   Joining data frames

# Getting started

Click on your assignment link, clone it in Posit and open the R Markdown document.
...
We'll use the **tidyverse** package for much of the data wrangling and visualisation and the **leaflet** package which we can use to make interactive graphics.

You can load them by running the following in your Console.
Don't forget to run `install.packages("leaflet")` if it is your first time using the package:
...
The datasets we'll use are called `dennys` and `laquinta`.
Note that these data were scraped from [the Denny's location website](https://locations.dennys.com/) and [the hotel listings on find and book](https://www.lq.com/en/findandbook/hotel-listings.html), respectively.
...
You can also find out more information on the datasets in the [dsbox documentation for the dennys data](https://rstudio-education.github.io/dsbox/reference/dennys.html) and the [dsbox documentation for the data](https://rstudio-education.github.io/dsbox/reference/laquinta.html).
...
2.  ✏️ What are the dimensions of the La Quinta's dataset?
...
8.  ✏️ Add a country variable to the La Quinta dataset.
...
```{r}
establishments <- bind_rows(dennys, laquinta)
```
...
```{r}
ggplot(establishments, mapping = aes(x = longitude, y = latitude, color = establishment)) +
  geom_point()
```

We can get a sense of the United States in this plot, but it would be better if we could look at this in context.
Adapt the code to complete the leaflet map.

```{r}

pal <- colorFactor(
  palette = c("red", "turquoise"), # Feel free to change the colors or choose a palette
  domain = establishments$establishment)

leaflet(establishments) %>%
  addProviderTiles(provider = providers$OpenStreetMap) %>% # change the provider with ?provider
  addCircleMarkers(lng = ~longitude, 
                   lat = ~latitude, 
                  # popup = ~establishment,
                  # color = ~pal(),
                   radius = ~ifelse(establishment == "Denny's", 1, 2),
                   opacity = 0.8,
                   group = ~establishment) %>%
#  addLegend(position = "bottomright",
#            pal = pal,
#            values = ~, # color by the establishment
#            title = "Establishment",
#            opacity = 1) %>%
  addLayersControl(
    overlayGroups = ~establishment,
...
11. ✏️ Filter the data for observations in North Carolina only, and recreate the leaflet plot.
...
12. ✏️ Now filter the data for observations in Texas only, and recreate the leaflet plot, with an appropriate `alpha` level.
    Visually, does Mitch Hedberg's joke appear to hold here?

That's it for now!
Feel free to customize your leaflet map further.
