...
1.  ✏️ Plot, using histograms, the distribution of plastic waste per capita faceted by continent. How do the continents compare to each other in terms of their plastic waste per capita?
...
2.  ✏️ Describe why we defined the `color` and `fill` of the curves by mapping aesthetics of the plot but we defined the `alpha` level as a characteristic of the plotting geom.
...
1.  ✏️ Convert your side-by-side box plots from the previous task to [violin plots](http://ggplot2.tidyverse.org/reference/geom_violin.html). What do the violin plots reveal that box plots do not? What features are apparent in the box plots but not in the violin plots?

```{marginfigure}
**Remember:** We use `geom_point()` to make scatterplots.
```

1.  ✏️ Visualize the relationship between plastic waste per capita and mismanaged plastic waste per capita using a scatterplot.
    Describe the relationship.

2.  ✏️ Colour the points in the scatterplot by continent.
...
3.  ✏️ Visualize the relationship between plastic waste per capita and total population as well as plastic waste per capita and coastal population.
...
1.  ✏️ Recreate the following plot, and interpret what you see in context of the data.
