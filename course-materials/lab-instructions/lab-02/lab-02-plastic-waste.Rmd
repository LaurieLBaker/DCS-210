---
title: "Lab 02 - Global plastic waste backup"
output: 
  tufte::tufte_html:
...
After you've clicked on your assignment link go ahead and open up the assignment repo.

Grab the URL of the repo, and clone it in RStudio.
Refer to [HW 01](https://laurielbaker.github.io/DCS-210/course-materials/hw-instructions/hw-01/hw-01-pet-names.html) if you would like to see step-by-step instructions for cloning a repo into an RStudio project.
...
If it is a data error, you will want to filter out this observation for the following exercises.

1.  :pencil: Plot, using histograms, the distribution of plastic waste per capita faceted by continent. How do the continents compare to each other in terms of their plastic waste per capita?
...
2.  :pencil: Describe why we defined the `color` and `fill` of the curves by mapping aesthetics of the plot but we defined the `alpha` level as a characteristic of the plotting geom.
...
1.  :pencil: Convert your side-by-side box plots from the previous task to [violin plots](http://ggplot2.tidyverse.org/reference/geom_violin.html). What do the violin plots reveal that box plots do not? What features are apparent in the box plots but not in the violin plots?

```{marginfigure}
**Remember:** We use `geom_point()` to make scatterplots.
```

1.  :pencil: Visualize the relationship between plastic waste per capita and mismanaged plastic waste per capita using a scatterplot.
    Describe the relationship.

2.  :pencil: Colour the points in the scatterplot by continent.
...
3.  :pencil: Visualize the relationship between plastic waste per capita and total population as well as plastic waste per capita and coastal population.
...
We don't expect you to complete all of the exercises within the lab session.
...
1.  :pencil: Recreate the following plot, and interpret what you see in context of the data.
