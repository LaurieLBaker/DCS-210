...
Once you've opened up RStudio Cloud, we will want to introduce RStudio to git and set up our personal access token, these replaced passwords in 2021.
...
Double check that your YAML output type is `html_document`.
If it is another output type, e.g. `github_document`, change it to `html_document` and save your changes.

```{r doublecheck_yaml, fig.width = 2, fig.height = 1.5, echo = FALSE}
knitr::include_graphics("img/yaml-double-check.png")
```

Click on the gear icon in on top of the R Markdown document, next to the blue knit icon 🧶, and select "Output Options..." in the dropdown menu.
