...
rmds <- dir_info(recurse = 3, glob = "course-materials/hw-instructions/hw-01/hw-01-pet-names.Rmd") %>% 
  filter(!str_detect(path, "slides")) %>%
  pull(path)
walk(rmds, render)

rmds <- dir_info(recurse = 3, glob = "course-materials/lab-instructions/lab-01/lab-01-hello-r.Rmd") %>% 
  filter(!str_detect(path, "slides")) %>%
  pull(path)
walk(rmds, render)
