...
Follow the assignment link and clone the project in RStudio and open the R Markdown document.
...
Since the dataset is distributed with the package.
It is also available in the data folder.
...
5.  We are going to do a little extra preproccessing using a recipe with the following steps for predicting `advfront` from `polviews`, `wrkstat`, and `educ`.
...
```{r eval=FALSE}
gss16_rec_1 <- recipe(advfront ~ factor(polviews) + factor(wrkstat) + educ, data = gss16) %>%
