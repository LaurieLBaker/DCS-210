...
-   are a member of the course Posit Cloud space
...
```{r clone-repo-link, fig.margin = TRUE, echo = FALSE, fig.alt = "Github repo code pane: navigate to code, select https, click clipboard to clone", eval = TRUE}
...
Go to [posit.cloud](https://posit.cloud/ "Posit Cloud") and then **navigate to the course workspace** via the left sidebar.
...
-   It's only when you're in the course workspace that your usage of Posit Cloud won't count towards the free usage limits.
...
Once you've opened up Posit Cloud, we will want to introduce set up our personal access token, these replaced passwords in 2021.
If you have already done this in the lab, skip ahead to step 6.
Our personal access token will connect our project to our github account.
This is perhaps the most tedious part of the process as you have to (re)introduce yourself to git *every time you make a new project*.
...
```{r check-git-sitrep, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/check-git-sitrep-1.png")
```
...
Add a description (e.g. course name) and update the expiry date to never expire.
This will mean that it does not need to be updated during the course.

```{r add-new-pat-2, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/add-new-pat-2.png")
```
...
```{r add-new-pat-3, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/add-new-pat-3-generate-token.png")
```

### Step 5: Copy and save generated token

Copy and save your generated token by clicking on the clipboard symbol.
Keep your token somewhere you can find it later, like in a text file in your course folder.
Note that for more sensitive projects you might consider using a password manager.

```{r add-new-pat-4-token, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/add-new-pat-4-token-generated.png")
```
...
```{r set-git-creds, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/set-git-creds-add-pat-1.png")
```
...
```{r recheck-git-sitrep, echo = FALSE, eval = TRUE}
knitr::include_graphics("img/recheck-git-sitrep-1.png")
```

Note that for future projects you can skip directly to Step 6.
You do not need to generate a personal access token each time.
...
Your commits will be the trail of breadcrumbs you leave in your analysis that allow you to retrace your steps.
Version control is about tracking **who** made **what** change and **why**.
Your github account capture the **who**, your commit message should capture **what** was done and when it is not obvious from the commit message, also the **why**.
If someone were to rerun the code committed, the message should describe what that code will do.

Your messages should follow the guidance from the blog post, [seven rules of a great GIT commit message](https://chris.beams.io/posts/git-commit/), to construct your message.
In this course we will be mainly using rules 3-5, but you might be interested in reading about the others.

-   Rule 3: Capitalize the subject line: **C**omplete Exercise 6
-   Rule 4: Do not end the subject line with a period
-   Rule 5: Use the imperative mood[^hw-01-pet-names-original-1] in the subject line: **Complete** Exercise 6
...
**Thought Exercise**: Why follow rules?
Why can't I write whatever I want?[^hw-01-pet-names-original-2]
...
This will prompt a dialogue box where you first need to enter your user name, and then your personal access token (PAT).
This might feel cumbersome.
I recommended keeping your PAT somewhere handy on your computer like on a sticky note on your desktop so it is easy to access.

**Thought exercise:** Which of the above steps (updating the YAML, committing, and pushing) needs to talk to GitHub?[^hw-01-pet-names-original-3]
...
[^hw-01-pet-names-original-1]: Spoken or written as if giving a command or instruction, e.g. "Clean your room!"

[^hw-01-pet-names-original-2]: There's nothing stopping you from writing what you want in a commit message.
    Following shared rules and standards provide consistency and promote clarity and shared understanding allowing your future self and collaborators to understand what was done and why.

[^hw-01-pet-names-original-3]: Only pushing requires talking to GitHub, this is why you're asked for your password at that point.
